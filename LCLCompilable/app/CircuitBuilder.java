...
public class CircuitBuilder {
	...
	private HashMap<Character,Boolean> outputs;//Holds the values of the outputs of the constructed circuits.
	...
	{
		mapOperators();
		Scanner inputScanner = new Scanner(inputDeclarations);
		while(inputScanner.hasNext())
		{
			String inputDec = inputScanner.next();
			constructInput(inputDec);
		}
		//System.out.println("Finished building inputs");
		inputScanner.close();
		
		String tempCircuitDec = "";
		for(int i =0;i<circuitDeclarations.length();i++)
		{
			char c = circuitDeclarations.charAt(i);
			if(c!=')'&&c!='(')
				tempCircuitDec +=c;
		}

		Scanner circuitStatements = new Scanner(tempCircuitDec);
	...
	public void buildDynamicCircuits(String inputDec,String circuitDec) throws Exception
	{
		...
		try
		{
			...
			String tempCircuitDec = "";
			for(int i =0;i<circuitDec.length();i++)
			{
				char c = circuitDec.charAt(i);
				if(c!=')'&&c!='(')
					tempCircuitDec +=c;
			}
			circuitScanner = new Scanner(tempCircuitDec);
		...
		//If no error is found then all class attributes will be changed with the temporary or 'dynamic' ones.	
